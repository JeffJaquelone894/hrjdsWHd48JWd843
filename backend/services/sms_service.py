...
async def send_application_accepted_sms(phone: str, name: str) -> dict:
    ...
    message = f"Hallo {name}! Ihre Bewerbung bei Nexura wurde akzeptiert. Bitte loggen Sie sich ein, um Ihren Arbeitsvertrag zu unterschreiben."
...
async def send_account_unlocked_sms(phone: str, name: str) -> dict:
    ...
    message = f"Willkommen bei Nexura, {name}! Ihr Account wurde freigeschaltet. Sie koennen sich jetzt im Mitarbeiterportal einloggen."
    return await send_sms(phone, message)
